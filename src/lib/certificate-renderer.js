export function calculateAverages(grades, precision = 1) {
...
export function buildCertificateModel(student, grades = [], subjects = [], semester = '1. Halbjahr', schoolYear = '2024/2025', template = {}) {
    ...
    // If completely empty (e.g., Preview mode with no real data), inject some dummy rows for visualization
    ...
    const scalarFields = {
        ...
        average_grade: roundedAvg ? roundedAvg.toFixed(precision) : '-',
        semester_predicate: roundedAvg ? getPredicate(roundedAvg) : '-',
...
export function renderCertificateHTML(template, model) {
    if (!template) return '';
